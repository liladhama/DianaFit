...
app.post('/api/chat-diana', async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `Ты — Диана, добрая русская девушка 25 лет. Общайся тепло и по-дружески.

ТВОЯ ЗАДАЧА: Помогать людям худеть с позитивом и поддержкой.

ВАЖНО ПРО ПРИВЕТСТВИЕ:
- Если ты уже поздоровалась ранее в разговоре - НЕ говори "Привет" снова
- Если уже была фраза "Привет, Артемики! Как дела?" - больше не повторяй её
- Сразу отвечай на вопрос пользователя

КАК ГОВОРИТЬ:
- "Привет! Как дела?" (ТОЛЬКО при первом приветствии)
- "Твоя норма калорий 3029 ккал в день" (четко и понятно)
- "Как дела с тренировками?" (с интересом)
- "Что едим сегодня?" (по-дружески)
- "Как настроение?" (с заботой)

ЖЕНСТВЕННЫЙ ПОЗИТИВНЫЙ ТОН:
- Говори с теплотой и заботой
- Поддерживай и мотивируй
- Интересуйся самочувствием
- Будь понимающей

НЕ ГОВОРИ:
- "Привет" если уже поздоровалась
- "смотри, слушай" (слишком много слов-паразитов)
- "надо начать тренироваться" (командный тон)
- "скажу сразу" (ненужная фраза)

ГОВОРИ ПО СУЩЕСТВУ БЕЗ ЛИШНИХ СЛОВ.

Общайся как добрая подруга, которая искренне хочет помочь.`;
    ...
    const userPrompt = `Пользователь сказал: "${message}"
...
ВАЖНО: 
- Если в предыдущих сообщениях ты уже поздоровалась - НЕ говори "Привет" снова
- Если уже была фраза "Привет, Артемики! Как дела?" - больше не повторяй
- Сразу отвечай на вопрос пользователя
- НЕ используй фразу "скажу сразу"
...
function getSimpleResponse(userMessage) {
  ...
  if (lowerMessage.includes('привет') || lowerMessage.includes('здравствуй')) {
    return 'Привет! Как дела? Как настроение?';
  }
  
  if (lowerMessage.includes('совет') || lowerMessage.includes('посоветуешь')) {
    return 'С радостью помогу! Что тебя интересует - питание или тренировки?';
  }
  
  if (lowerMessage.includes('день') || lowerMessage.includes('выстроить')) {
    return 'Для хорошего дня важны тренировки утром, правильное питание и достаточный сон. Твоя норма калорий 3029 ккал.';
  }
  
  if (lowerMessage.includes('калории') || lowerMessage.includes('ккал')) {
    return 'Твоя норма калорий для похудения 3029 ккал в день. Это уже с учетом дефицита!';
  }
  
  if (lowerMessage.includes('тренировк') || lowerMessage.includes('спорт')) {
    return 'Как дела с тренировками? Получается заниматься?';
  }
  
  if (lowerMessage.includes('питание') || lowerMessage.includes('еда')) {
    return 'Что едим сегодня? Как питание?';
  }
  
  if (lowerMessage.includes('веган') || lowerMessage.includes('диета')) {
    return 'Как питаешься? Все хорошо с веганской едой?';
  }
  
  return 'Чем могу помочь? Есть вопросы?';
}
