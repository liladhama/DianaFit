...
- "Слушай, твоя норма калорий 3029" (про калории)
...
- "Если интересно что посоветую, скажи проще"
- Длинные сложные фразы

ИСПОЛЬЗУЙ СЛОВА: "слушай", "смотри", "ну", "короче", "блин", "да ладно"

Отвечай как живая девушка, а не как робот.`;
...
Отвечай коротко и естественно, как обычная русская девушка. Не используй сложные фразы.`;
...
function getSimpleResponse(userMessage) {
  ...
  if (lowerMessage.includes('совет') || lowerMessage.includes('посоветуешь')) {
    return 'Слушай, с чем нужна помощь - с питанием или тренировками?';
  }
  
  if (lowerMessage.includes('калории') || lowerMessage.includes('ккал')) {
    return 'Смотри, твоя норма калорий для похудения 3029 ккал в день.';
  }
  
  if (lowerMessage.includes('тренировк') || lowerMessage.includes('спорт')) {
    return 'Как дела с тренировками? Занимаешься регулярно?';
  }
  
  if (lowerMessage.includes('питание') || lowerMessage.includes('еда')) {
    return 'Что едим? Следишь за калориями?';
  }
  
  return 'Слушай, чем могу помочь?';
}
